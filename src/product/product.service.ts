...
export class ProductService {
  ...
  create(dto: CreateProductDto) {
    return this.product.save({
      ...
      simularProjects: dto.simularProjects,
      colors: dto.colors,
  ...
  async checkProductInFavorite(userId: number, productId: number) {
    const product = await this.product.findOne({
      where: { id: productId },
      relations: ['simularProducts', 'sizes'],
    });
    ...
    const didUserExist = await user.getOne();
    if (!didUserExist) {
      throw new NotFoundException('User not found');
    }

    const inFavorite = await user
      .leftJoinAndSelect('user.favorites', 'favorites')
      .andWhere('favorites.id = :productId', { productId })
      .getOne();

    if (inFavorite) return true;
    return false;
  }
  ...
  async findOne(userId: number, productId: number) {
    ...
    if (userId) {
      ...
      if (inFavorite) product.inFavorite = true;
    }
